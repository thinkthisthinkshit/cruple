...
function AuthorProfile() {
  ...
  const handleStartChat = async () => {
    ...
    if (user.username === username) {
      console.error("Cannot start chat with self:", username);
      toast.error("Нельзя начать чат с собой");
      return;
    }
    console.log("Starting chat with:", username, "User:", user.username, "Token:", user.token.substring(0, 10) + "...");
    console.log("Request to POST /messages/start, payload:", { username: username.trim() });
    try {
...
function Messages() {
  ...
  useEffect(() => {
    ...
    if (startChatWith) {
      ...
      axios
        .get(`http://localhost:3000/users/${startChatWith}`)
        .then(() => {
...
function Chat({ chatId, recipient, onMessageSent }) {
  ...
  const sendMessage = async () => {
    console.log("sendMessage called with newMessage:", newMessage);
    if (!newMessage.trim()) {
